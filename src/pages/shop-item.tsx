    ...
    <>
      <div className="container h-screen flex flex-col bg-gray-100 justify-between items-stretch m-auto ">
        ...
        <div className="px-3 py-2 flex flex-row justify-between items-center gap-2 bg-gray-400">
          <IconButton badge={items.length} label="購物車" icon="ShoppingCart" />
          <div className="flex flex-row flex-grow gap-2">
            <Button onClick={handleOpenBottomSheet} className="flex-grow" variants={{ hierarchy: "secondary" }}>
              加入購物車
            </Button>
            <Button onClick={handleOpenBottomSheet} className="flex-grow">直接購買</Button>
          </div>
        </div>
      </div>
      <BottomSheet open={isBottomSheetOpen} onClose={() => setIsBottomSheetOpen(false)} data={mockShopItem} />
    </>
  )
}
