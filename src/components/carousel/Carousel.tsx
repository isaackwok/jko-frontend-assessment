      ...
      {imgUrls.length ? (
        <>
          ...
          <div
            ...
            onClick={() => setCurrentImgIdx((i) => Math.max(i - 1, 0))}
          />
          <div
            className="absolute inset-y-0 right-0 w-1/2"
            onClick={() =>
              setCurrentImgIdx((i) => Math.min(i + 1, imgUrls.length - 1))
            }
          />
          <span className="absolute text-white text-p3 rounded-full right-3 bottom-3 py-1 px-2 bg-black/30">
